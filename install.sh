...
# Function to add CAN0 & Bitrate 100000 (100K) if not exists
can0_service_add() {
    echo
    echo "     Adding Can0 configuration..."
    local can0_conf="/etc/systemd/network/80-can.network"
    if ! grep -q "Name=can0" "$can0_conf"; then
        cat > "$can0_conf" << EOF
...
bluetooth_receiver() {
	...
	if ! grep -q "Class = 0x200414" "$main_conf"; then
		sed -i 's/^#\?Class = 0x200414/Class = 0x200414/' "$main_conf"
...
kodi_configure() {
    ...
    if grep -q 'screensaver.xbmc.builtin.dim' /home/pi/.kodi/userdata/guisettings.xml; then
        echo "     Setting screensaver to none in guisettings.xml..."
        sed -i 's/ default="true">screensaver.xbmc.builtin.dim/> /' /home/pi/.kodi/userdata/guisettings.xml
    ...
    if ! grep -q 'volumeamplification>30.000000' /home/pi/.kodi/userdata/guisettings.xml; then
    ...
    if grep -q 'services.webserver' /home/pi/.kodi/userdata/guisettings.xml; then
    ...
    if grep -q 'webserverauthentication" default="true' /home/pi/.kodi/userdata/guisettings.xml; then
    ...
    cat > /home/pi/.kodi/userdata/sources.xml << 'EOF'
<sources>
    <programs>
...
EOF
    ...
    echo "     Creating media directories under /home/pi/..."
    mkdir -p /home/pi/movies
    mkdir -p /home/pi/tvshows
    mkdir -p /home/pi/clips
...
if whiptail --title "Video Output" --yes-button " HDMI " --no-button " JACK " --yesno "Select HDMI-VGA or 3.5mm Jack for as the video output source." 10 60; then
...
# Interactive IR-Remote control (using whiptail)
if whiptail --title "IR Remote Control TSOP4838" --yesno "          Turn on the IR receiver for control? \n\n         ATTENTION!!!! ONLY FOR RNS-JP3 (Asian)" 10 60; then
	rpi_ir
	green "     Installed IR-Keytable"
else
	sed -i '/gpio-ir/d' $CONFIG # Del GPIO IR 🗑️
	red "     Disable IR Remote to $CONFIG. ✅"
fi
